...
def crop_image(img: Image.Image, crop_size: int = 90) -> Image.Image:
...
def crop_around_center(img, center, crop_size=90):
    """Crop a PIL image to crop_size x crop_size."""
...
def analyze_model_experiments(result_df):
    # On reprend juste les codes de la question précédente mais en modifiant le nom du tableau
     # On cherche classificateurs avec mauvaise performance de test (sous la médiane)
    tebad = result_df['perf_te'] < result_df['perf_te'].median()
    # On cherche classificateurs avec mauvaise performance d'entraînement (sous la médiane)
    trbad = result_df['perf_tr'] < result_df['perf_tr'].median()
    # Surapprentissage=bonne perf train, mauvaise perf test
    overfitted = tebad & ~trbad
    # Sous apprentissage= mauvaise perf train et mauvaise perf test
...
# LES FONCTIONS / ELEMENTS QU'ON A AJOUTER : 

def brightness(img):
    """Luminosité moyenne des pixels de l'avant-plan : moyenne de (R+G+B)/3.
    
    Un téléphone allumé a un écran lumineux (proche de 255 ==> clair),
    un téléphone éteint a un écran noir (proche de 0 ==> sombre).
    C'est un attribut très pertinent pour notre jeu de donnée.
    """
    M = np.array(img)
    F = M[:, :, 3] > 0  # pixels de l'avant-plan (canal alpha)
    V = (M[:, :, 0] * 1.0 + M[:, :, 1] * 1.0 + M[:, :, 2] * 1.0) / 3.0
    return np.mean(V[F])


def std_brightness(img):
    """Écart-type de la luminosité des pixels de l'avant-plan.
    L’écart-type de la luminosité permet de mesurer la variabilité des pixels. 
    Un écran allumé présente une forte variabilité tandis qu’un écran éteint est uniforme, et donc avec un écart-type faible.
    """
    M = np.array(img)
    F = M[:, :, 3] > 0
    V = (M[:, :, 0] * 1.0 + M[:, :, 1] * 1.0 + M[:, :, 2] * 1.0) / 3.0
...
# Attributs du Projet 1 (réutilisés et adaptés)

def grayscale(img: Image.Image) -> np.ndarray:
    """Return image in gray scale (moyenne des canaux R, G, B)."""
    return np.mean(np.array(img)[:, :, :3], axis=2)


class MatchedFilter:
    """Filtre adaptatif (matched filter) pour extraire un attribut
    par corrélation avec un template moyen.

    Implémenté lors du Projet 1 (smileys), réutilisé ici pour les téléphones.
    On crée un template moyen en niveaux de gris à partir d'un ensemble d'exemples,
    puis on mesure la corrélation de chaque image avec ce template.
    """

    def __init__(self, examples):
        """Crée le template moyen à partir d'une série ou liste d'images PIL."""
        M = np.mean([grayscale(img) for img in examples], axis=0)
        self.template = (M - M.mean()) / M.std()

    def match(self, img: Image.Image) -> float:
        """Retourne la corrélation entre l'image et le template.

        Une valeur proche de 1 indique que l'image ressemble au template,
        une valeur proche de -1 indique qu'elle lui est opposée.
        """
        M = grayscale(img)
        M = (M - M.mean()) / (M.std() + np.finfo(float).eps)
        return float(np.mean(np.multiply(self.template, M)))
